"""Project setup / ensure helpers (importable library surface).

Extracted from the transitional in-tree CLI so domain modules and product
templates can share pure helpers and ensure logic. Payload CMS-oriented
defaults remain for fidelity with the current public template; product-
specific command wrappers land in later modules.
"""
...
from xgic.cli.core.docker import DEFAULT_CONFIG_FILE
...
def ensure_payload_project(*, quiet: bool = False) -> int:
    ...
    Idempotent: complete project returns 0 with no output. Creation failures
    are best-effort (non-zero only for early unrecoverable cases).
    """
    ...
    if not quiet:
        ...
        print_info(
            ...
            "Re-run project ensure via the Payload CMS CLI module when available "
